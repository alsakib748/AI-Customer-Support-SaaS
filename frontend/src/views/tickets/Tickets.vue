...
const loadData = async () => {
    await Promise.all([ticketStore.fetchTickets({ ...filters }), ticketStore.fetchStatistics(), customerStore.fetchCustomers({ per_page: 100 }), conversationStore.fetchConversations({ per_page: 50 }), teamStore.fetchMembers()]);
};
    ...
    <div class="p-6">
        ...
        <div class="mb-6 flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <Button v-if="canCreateTickets" label="New Ticket" icon="pi pi-plus" severity="primary" @click="openCreateDialog" />
        ...
        <div class="mb-4 flex flex-wrap gap-3 items-center">
            <div class="flex-1 min-w-[200px]">
                <InputText v-model="filters.search" placeholder="Search tickets..." class="w-full" @input="applyFilters" />
            </div>
            <div class="w-40">
            ...
            <div class="w-40">
                <Select v-model="filters.type" :options="typeOptions" optionLabel="label" optionValue="value" placeholder="Type" class="w-full" @change="applyFilters" clearable />
            </div>
            ...
            <Column header="Actions" style="width: 250px">
                <template #body="{ data }">
                    <div class="flex gap-1 flex-wrap">
                        <Button icon="pi pi-eye" severity="info" text rounded @click="viewTicket(data)" tooltip="View Details" />
                        ...
                        <Button
                            ...
                            text
                            rounded
                            @click="unassignTicket(data)"
                            tooltip="Unassign"
                        />

                        <Button v-if="canUpdateTickets && data.status === 'open'" icon="pi pi-play" severity="success" text rounded @click="startTicket(data)" tooltip="Start" />

                        <Button v-if="canUpdateTickets && (data.status === 'open' || data.status === 'in_progress' || data.status === 'pending')" icon="pi pi-check" severity="success" text rounded @click="resolveTicket(data)" tooltip="Resolve" />

                        <Button v-if="canUpdateTickets && (data.status === 'resolved' || data.status === 'closed')" icon="pi pi-refresh" severity="warning" text rounded @click="reopenTicket(data)" tooltip="Reopen" />

                        <Button v-if="canDeleteTickets" icon="pi pi-trash" severity="danger" text rounded @click="confirmDelete(data)" tooltip="Delete" />
                    </div>
                </template>
            </Column>
        </DataTable>
        ...
        <Dialog v-model:visible="showCreateDialog" header="New Ticket" :style="{ width: '600px' }" modal>
            <form @submit.prevent="handleCreate" class="space-y-4">
                ...
                <div>
                    ...
                    <InputText v-model="createForm.subject" class="w-full" :class="{ 'p-invalid': getFieldError('subject') }" placeholder="Brief description of the issue" />
                ...
                <div>
                    ...
                    <Textarea v-model="createForm.description" class="w-full" rows="3" placeholder="Detailed description of the issue" />
                </div>
                ...
                <div>
                    ...
                    <Select v-model="createForm.type" :options="typeOptions" optionLabel="label" optionValue="value" class="w-full" placeholder="Select Type" />
                </div>

                <div>
                    <label class="block text-sm font-medium mb-1">Conversation (Optional)</label>
                    <Select v-model="createForm.conversation_id" :options="conversations" optionLabel="subject" optionValue="id" class="w-full" placeholder="Select Conversation" filter />
                </div>

                <div>
                    <label class="block text-sm font-medium mb-1">Assign To</label>
                    <Select v-model="createForm.assigned_user_id" :options="teamMembers" optionLabel="name" optionValue="id" class="w-full" placeholder="Select Agent" filter />
                </div>
            </form>

            <template #footer>
                <Button label="Cancel" icon="pi pi-times" severity="secondary" @click="showCreateDialog = false" />
                <Button label="Create Ticket" icon="pi pi-plus" severity="primary" :loading="saving" @click="handleCreate" />
            </template>
        </Dialog>

        <!-- Ticket Details Dialog -->
        <Dialog v-model:visible="showDetailsDialog" header="Ticket Details" :style="{ width: '800px' }" modal :maximizable="true">
            <div v-if="selectedTicket" class="space-y-4">
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex gap-2">
                        ...
                        />
                        <Button v-if="canUpdateTickets && selectedTicket.status === 'open'" icon="pi pi-play" label="Start" severity="success" size="small" @click="startTicket(selectedTicket)" />
                        <Button
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <div class="text-sm" v-if="selectedTicket.resolved_at">Resolved: {{ formatDate(selectedTicket.resolved_at) }}</div>
                        <div class="text-sm" v-if="selectedTicket.closed_at">Closed: {{ formatDate(selectedTicket.closed_at) }}</div>
                        <div class="text-sm text-danger" v-if="selectedTicket.is_overdue">⚠️ Overdue</div>
                    </div>
                </div>
                ...
                <div v-if="selectedTicket.conversation">
                    ...
                    <div class="mt-1">
                        <Button icon="pi pi-comments" :label="'Conversation #' + selectedTicket.conversation.id" severity="info" outlined size="small" @click="viewConversation(selectedTicket.conversation.id)" />
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <Button icon="pi pi-plus" label="Add Comment" severity="primary" size="small" @click="showCommentDialog = true" />
                    </div>
                    <div class="space-y-2 max-h-60 overflow-y-auto">
                        ...
                        >
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <Tag :value="comment.type_label" :severity="comment.is_internal ? 'warning' : 'info'" size="small" />
                                </div>
                        ...
                        <div v-if="!comments.length" class="text-center text-surface-400 text-sm py-4">No comments yet</div>
                    </div>
                </div>
            </div>

            <template #footer>
                <Button v-if="selectedTicket && canUpdateTickets && selectedTicket.status !== 'closed'" icon="pi pi-times" label="Close" severity="secondary" @click="closeTicket(selectedTicket)" />
        ...
        <Dialog v-model:visible="showCommentDialog" header="Add Comment" :style="{ width: '500px' }" modal>
            <div class="space-y-4">
                <div>
                    ...
                    <Select v-model="commentForm.type" :options="commentTypeOptions" optionLabel="label" optionValue="value" class="w-full" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1">Content *</label>
                    <Textarea v-model="commentForm.content" class="w-full" rows="4" placeholder="Write your comment..." />
                </div>
            </div>
            <template #footer>
                <Button label="Cancel" icon="pi pi-times" severity="secondary" @click="showCommentDialog = false" />
                <Button label="Add Comment" icon="pi pi-plus" severity="primary" :loading="saving" @click="handleAddComment" :disabled="!commentForm.content.trim()" />
            </template>
        </Dialog>
