...
const sendMessage = async (messageContent) => {
    ...
    try {
        const result = await messageStore.sendMessage(currentConversation.value.id, { content: messageContent.trim() });
        ...
        <div class="flex-1 flex flex-col">
            ...
            <div v-if="currentConversation" class="p-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button v-if="canReply && (currentConversation.status === 'open' || currentConversation.status === 'pending')" icon="pi pi-check" label="Resolve" severity="success" size="small" @click="handleResolve" />
            ...
            <div v-if="currentConversation" class="flex-1 overflow-y-auto p-4 space-y-2" ref="messagesContainer">
                ...
                <AIStreamingMessage v-if="isStreaming && streamingContent" :content="streamingContent" :is-streaming="true" />

                <!-- Empty State -->
                <div v-if="!loadingMessages && messages.length === 0 && !isStreaming" class="text-center py-12 text-surface-500">
            ...
            <!-- Message Composer -->
            <MessageComposer v-if="currentConversation && canReply" @send="sendMessage" @addNote="showNoteDialog = true" :disabled="currentConversation.status === 'closed' || isStreaming" :sending="sending || isStreaming" />
