    ...
    <div class="flex h-[calc(100vh-120px)] bg-surface-50 dark:bg-surface-950">
        ...
        <div class="w-80 border-r border-surface-200 dark:border-surface-700 flex flex-col">
            <div class="p-4 border-b border-surface-200 dark:border-surface-700">
                ...
                <InputText v-model="conversationFilters.search" placeholder="Search..." class="w-full mt-2" @input="searchConversations" />
            </div>
            <div class="flex-1 overflow-y-auto">
                ...
                >
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1 ml-2">
                            ...
                            <Tag v-else-if="conv.status === 'pending'" value="" severity="warning" rounded class="w-2 h-2 p-0" />
                ...
                <div v-if="!loadingConversations && conversations.length === 0" class="p-8 text-center text-surface-500">
        ...
        <div class="flex-1 flex flex-col">
            <!-- Chat Header -->
            <div v-if="currentConversation" class="p-4 border-b border-surface-200 dark:border-surface-700 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <Avatar :label="currentConversation.customer?.full_name?.charAt(0) || '?'" shape="circle" size="large" />
                    <div>
                        <div class="font-bold">{{ currentConversation.customer?.full_name || 'Unknown' }}</div>
                        <div class="text-sm text-surface-500">{{ currentConversation.customer?.email || 'No email' }}</div>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <Button v-if="canReply && currentConversation.status === 'open'" icon="pi pi-check" label="Resolve" severity="success" size="small" @click="handleResolve" />
                    <Button v-if="canReply && (currentConversation.status === 'resolved' || currentConversation.status === 'closed')" icon="pi pi-refresh" label="Reopen" severity="warning" size="small" @click="handleReopen" />
                    <Button icon="pi pi-ellipsis-v" severity="secondary" text rounded @click="showActionsMenu = !showActionsMenu" />

                    <Button v-if="canReply && currentConversation?.status !== 'closed'" icon="pi pi-times" label="Close" severity="secondary" size="small" @click="handleClose" />
                </div>
            </div>
            ...
            <div v-if="currentConversation" class="flex-1 overflow-y-auto p-4 space-y-2" ref="messagesContainer">
                ...
                <template v-for="message in messages" :key="message.id">
                    ...
                    <div v-if="message.is_internal_note" class="flex justify-center my-2">
                        <div class="bg-yellow-50 dark:bg-yellow-950 text-yellow-800 dark:text-yellow-200 px-4 py-2 rounded-lg text-sm max-w-[80%]">
                            <div class="flex items-center gap-2">
                                <i class="pi pi-lock text-xs"></i>
                                <span class="font-medium">Internal Note</span>
                            </div>
                            <div class="mt-1">{{ message.content }}</div>
                            <div class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">{{ message.sender.name }} • {{ message.formatted_date }}</div>
                        </div>
                    </div>

                    <!-- System Event -->
                    <div v-else-if="message.sender.type === 'system'" class="flex justify-center my-2">
                        <div class="bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-400 px-4 py-2 rounded-lg text-sm">
                    ...
                    >
                        <div
                            class="flex items-start gap-2 max-w-[70%]"
                            :class="{
                                'flex-row-reverse': message.sender.type === 'agent'
                            }"
                        >
                            <Avatar
                                :label="message.sender.name?.charAt(0) || '?'"
                            ...
                            <div>
                                ...
                                <div
                                    class="rounded-lg px-4 py-2 mt-1 break-words"
                                    :class="{
                                        'bg-primary text-white': message.sender.type === 'agent',
            ...
            <div v-if="currentConversation && canReply" class="p-4 border-t border-surface-200 dark:border-surface-700">
                <div class="flex gap-2">
                    <div class="flex-1">
                        <Textarea v-model="newMessage" placeholder="Type a message..." class="w-full" rows="2" @keydown.ctrl.enter="sendMessage" @keydown.meta.enter="sendMessage" :disabled="sending || currentConversation.status === 'closed'" />
                    </div>
                    <div class="flex flex-col gap-2">
                        <Button icon="pi pi-send" label="Send" severity="primary" :loading="sending" @click="sendMessage" :disabled="!newMessage.trim() || sending || currentConversation.status === 'closed'" />
                        <Button icon="pi pi-lock" label="Add Note" severity="secondary" outlined size="small" @click="showNoteDialog = true" :disabled="currentConversation.status === 'closed'" />
                    </div>
                </div>
                <div class="text-xs text-surface-400 mt-1">Press Ctrl+Enter to send</div>
            </div>

            <!-- Conversation Closed Message -->
            <div v-else-if="currentConversation && currentConversation.status === 'closed'" class="p-4 border-t border-surface-200 dark:border-surface-700 text-center text-surface-500">
        ...
        <Dialog v-model:visible="showNoteDialog" header="Add Internal Note" :style="{ width: '500px' }" modal>
            <div class="space-y-4">
                <Textarea v-model="noteContent" placeholder="Write your internal note here..." class="w-full" rows="4" />
            ...
            <template #footer>
                ...
                <Button label="Add Note" icon="pi pi-plus" severity="primary" :loading="sending" @click="submitNote" :disabled="!noteContent.trim()" />
            </template>
        </Dialog>
