    ...
    <div class="p-6">
        ...
        <div v-else-if="analytics" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <template #content>
                        <ul class="space-y-2">
                            <li class="flex justify-between">
                                <span>Trialing</span><span class="font-bold">{{ analytics.trialing_tenants }}</span>
                            </li>
                            <li class="flex justify-between">
                ...
                <Card>
                    ...
                    <template #content>
                        <ul class="space-y-2">
                            <li class="flex justify-between">
                                <span>New Subscriptions</span><span class="font-bold text-success">{{ analytics.new_subscriptions }}</span>
                            </li>
                            <li class="flex justify-between">
                                <span>Churned</span><span class="font-bold text-danger">{{ analytics.churned_subscriptions }}</span>
                            </li>
                            <li class="flex justify-between">
                                <span>Revenue (30d)</span><span class="font-bold">{{ formatCurrency(analytics.revenue_last_30_days) }}</span>
                            </li>
                            <li class="flex justify-between">
                                <span>Payment Failures</span><span class="font-bold text-danger">{{ analytics.payment_failures_last_30_days }}</span>
                            </li>
                        </ul>
                    </template>
                </Card>
            ...
            <div class="flex gap-2 flex-wrap">
                ...
                <Button label="Subscriptions" icon="pi pi-users" outlined @click="router.push('/admin/billing/subscriptions')" />
                <Button label="Invoices" icon="pi pi-file" outlined @click="router.push('/admin/billing/invoices')" />
                <Button label="Payments" icon="pi pi-money-bill" outlined @click="router.push('/admin/billing/payments')" />
                <Button label="Coupons" icon="pi pi-ticket" outlined @click="router.push('/admin/billing/coupons')" />
                <Button label="Analytics" icon="pi pi-chart-pie" outlined @click="router.push('/admin/billing/analytics')" />
            </div>
        </div>
    </div>
</template>
